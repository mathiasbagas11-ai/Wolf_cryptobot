...
def create_app(application: Optional[Application] = None) -> FastAPI:
    ...
    @api.get("/paper")
    def paper() -> dict:
        if app_obj.paper is None:
            return {"enabled": False}
        return {"enabled": True, **app_obj.paper.stats()}
    ...
    def backtest(payload: dict = Body(default={})) -> dict:
        ...
        symbols = payload.get("symbols") or app_obj.screener._universe
        result = app_obj.backtest.run(symbols)
        # Drop the (non-serialisable) raw trade objects from the response.
