...
Trend-continuation entries on a pullback — the spirit of the old
``detect_swing_setup``. It requires an established trend (EMA alignment), a
healthy pullback toward the fast EMA, and a rejection candle in the trend
direction, while avoiding over-extended conditions.
...
class SwingDetector(Detector):
    ...
    def evaluate(self, symbol: str, candles: Sequence[Candle], context=None) -> Optional[SignalCandidate]:
        ...
        # 3. Rejection candle in trend direction
        ...
        # Entry at the fast EMA retest zone; wait for price to revisit it.
        entry = fast
        sl, tp, ladder = build_targets(entry, atr, is_long=is_long, sl_mult=2.0, tp_mults=(2.0, 4.0))
        # Guard: ATR geometry must keep TP/SL on the correct side of entry.
